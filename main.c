...
int main(int argc, char *argv[]){
    /*Input dos Program Argumets (pelo utilizador), ou seja, o utilizador informa o programa que file quer abrir*/
    ler_file(argc, argv);
...
void ler_file(int argc, char *argv[]){
    if (argc < 2) {
        aviso(argv[0]);
        exit(0);
    }
    ...
    if (fPointer == (FILE *) NULL) {
        fprintf(stderr, "O ficheiro %s n�o pode ser lido ou n�o existe.  Tente novamente.\n", argv[1]);
        exit(0);
    }
    char fileInfo[150];
